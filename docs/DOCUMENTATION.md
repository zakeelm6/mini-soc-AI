...
22. [Limites connues & perspectives](#22-limites-connues--perspectives)
...
### 5.9 Formules de scoring — détail technique

**Vecteur de features (11 dimensions)**, partagé par l'Isolation Forest et le Random Forest :

| Feature | Description | Domaine |
|---|---|---|
| `is_failed` | Échec d'authentification | {0, 1} |
| `is_invalid_usr` | Utilisateur inexistant | {0, 1} |
| `is_sshd_auth` | Contexte SSH authentification | {0, 1} |
| `has_src_ip` | Connexion distante (IP source présente) | {0, 1} |
| `is_session_ok` | Session ouverte avec succès | {0, 1} |
| `hour_sin` / `hour_cos` | Heure cyclique sin/cos(2π·h/24) | [-1, 1] |
| `is_night` | Nuit (0h–6h ou 22h–24h) | {0, 1} |
| `is_weekend` | Week-end | {0, 1} |
| `fail_ratio` | Ratio échecs/total par IP (baseline 7j) | [0, 1] |
| `login_velocity` | Vitesse de connexions normalisée | [0, 1] |

**Score hybride Isolation Forest** — combine le score IF brut et une règle SSH :

```
if_score  = min(1, (offset - raw) / max(|offset| × 0.1, 0.003))   si raw < offset, sinon 0
ssh_score = is_failed × has_src_ip × (1 + is_invalid_usr) × 0.45
score     = min(1.0, max(if_score, ssh_score))
```

**Boost CVE contextuel** dans l'ensemble — interroge `soc-cve-alerts` (cache 30 min), appliqué si au moins un modèle IA vote :

```
cve_boost = min(0.10, max(0, (CVSS - 7.0) / 30.0))
```

**Score de priorité d'incident** — combine sévérité, score IA et âge de l'incident :

```
P = 0.40 × w_sev + 0.35 × w_ia + 0.25 × w_age
```

**Mapping features → MITRE ATT&CK** (`ia_detector.py`) :

| Condition | Technique | ID |
|---|---|---|
| `is_session_ok=1, has_src_ip=1, is_failed=0` | Valid Accounts | T1078 |
| `is_failed=1, is_invalid_usr=1, has_src_ip=1` | Password Guessing | T1110.001 |
| `is_failed=1, login_velocity ≥ 0.3` | Credential Stuffing | T1110.004 |
| sinon | Brute Force | T1110 |

**Mapping votes → MITRE ATT&CK** (`ensemble_detector.py`) :

| Condition de vote | Technique | ID |
|---|---|---|
| vote_rate + rate ≥ 30 + 1 modèle IA | Credential Stuffing | T1110.004 |
| ≥ 2 modèles IA votent | Password Guessing | T1110.001 |
| vote_rate + 1 modèle IA | Brute Force | T1110 |
| RF seul (score ≥ 0.80) sans rate | Valid Accounts | T1078 |

**Meta-Learner (`meta_learner.py`)** — remplace les poids fixes de l'ensemble par un modèle appris sur les verdicts analystes (`llm_verdict` dans `soc-incidents`) :
- Features d'entrée : `[if_score, rf_score, dl_score, rate_score, votes]`
- Modèles testés : LogisticRegression, GradientBoosting, XGBoost
- Si disponible, remplace les poids fixes (0.30/0.35/0.20/0.15) ; sinon fallback sur les poids fixes

**Dernier calcul de métriques** (`meta_stats.json`, 153 échantillons) :

| Modèle | Précision | Rappel | F1 |
|---|---|---|---|
| Isolation Forest | 0.857 | 0.980 | 0.914 |
| Random Forest | 0.730 | 0.827 | 0.775 |
| Autoencoder DL | 0.606 | 0.612 | 0.609 |
| Rate Detector | 0.653 | 0.480 | 0.553 |
| **Meta-learner** | **1.000** | **1.000** | **0.989** |

Poids dynamiques appris par le meta-learner sur ce dataset : `w_if=0.321 · w_rf=0.272 · w_dl=0.214 · w_rate=0.194` — proches des poids fixes de départ, ce qui valide empiriquement le calibrage initial (0.30/0.35/0.20/0.15).
...
> Un App Password Gmail SMTP a été exposé accidentellement en clair pendant le développement (terminal/chat) — il a été révoqué immédiatement après détection.  
> Leçon retenue : ne jamais écrire de credentials en clair dans le terminal, le chat, ou le code versionné.  
> Utiliser `.env` exclusivement (voir `.env.example`).
...
### Résultats de détection — kill chain 9 étapes

| # | Technique | Action de l'attaquant | Détecté |
|---|---|---|---|
| 1 | T1110 Initial Access | 468 tentatives SSH (Hydra) | ✅ |
| 2 | T1059 Execution | Connexion réussie, exécution bash | ✅ |
| 3 | T1087 Discovery | whoami, uname, netstat, ps aux | ✅ |
| 4 | T1068 Privilege Escalation | sudo su | ✅ |
| 5 | T1053 Persistence | crontab backdoor, ~/.bashrc | ✅ |
| 6 | T1552 Credential Access | Lecture /etc/shadow (50 comptes) | ✅ |
| 7 | T1070 Defense Evasion | history -c | ✅ |
| 8 | T1048 Exfiltration | curl vers serveur externe, scp | ✅ |
| 9 | T1059 C2 | Reverse shell nc | ✅ |

**Taux de détection : 9/9 = 100%.** L'Ensemble Detector a créé 3 incidents critiques dans les 30 secondes suivant chaque changement de tactique ; les notifications e-mail ont été reçues en moins de 60 secondes.
...
## 22. Limites connues & perspectives

### Ce que c'est — et ce que ce n'est pas
Le Mini-SOC est un **prototype académique validé en environnement de laboratoire contrôlé**, avec un volume de logs limité et des scénarios d'attaque définis à l'avance (SSH brute force, scans web, injections SQL, kill chain de post-exploitation). Ce n'est pas un remplaçant direct d'un SIEM/SOC commercial en production — c'est une démonstration que l'essentiel des fonctionnalités d'un SOC (collecte, détection ML hybride, triage, réponse) peut être assemblé avec des briques 100% open source, à coût nul en licences.

### Reste pour un usage en production
- ⬜ TLS devant Flask (reverse proxy nginx/Caddy à configurer)
- ⬜ Chiffrement au repos Elasticsearch (`xpack.security.encryption`)
- ⬜ Services systemd pour les détecteurs (redémarrage automatique en cas de crash)
- ⬜ Cluster Elasticsearch multi-nœuds (haute disponibilité, tolérance aux pannes)
- ⬜ Gestion des secrets via un vault dédié plutôt que `.env`

### Perspectives fonctionnelles
- Intégration Suricata IDS pour corréler les alertes réseau avec les logs système/applicatifs
- Sources de logs supplémentaires : Windows Event Log, journaux applicatifs additionnels
- Détection d'attaques Active Directory (Kerberoasting, RDP suspect, mouvements latéraux)
- Enrichissement continu de la base RAG (playbooks, procédures internes, fiches MITRE)
- Amélioration du chatbot SOC pour assister davantage l'analyste dans l'interprétation des incidents
- Intégration TAXII/STIX pour le partage d'indicateurs de compromission
